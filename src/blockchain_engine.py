...
class BlockchainEngine:
    def __init__(self):
        self.rpc_url = os.getenv("RPC_URL", "https://polygon-amoy-bor-rpc.publicnode.com")
        ...
        if not self.w3.is_connected():
            raise ConnectionError(f"Failed to connect to Polygon Amoy RPC: {self.rpc_url}")
    ...
    def publish_verification_proof(self, composite_hash: str, metadata_url: str):
        ...
        # Check if record already exists in smart contract
        try:
            is_valid, _, _, _ = self.contract.functions.verifyRecord(bytes32_hash).call()
            if is_valid:
                return {
                    "tx_hash": "RECORD_ALREADY_STORED_IN_CONTRACT",
                    "block_number": "Existing On-Chain Record",
                    "status": 1,
        ...
        # Execute Smart Contract call: registerRecord(bytes32, string)
